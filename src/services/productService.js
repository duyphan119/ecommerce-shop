  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (type && type === "best-seller") {
        ...
        if (include) {
          option.include = defaultProductInclude(user, allDiscounts);
        }
...
const search = async (user, query) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const listDetail = await db.ProductDetail.findAll({
        nest: true,
        where: {
          [Op.or]: [
            {
            ...
            {
              "$product.category.group_category.name$": {
                [Op.substring]: q,
              },
            },
            {
              "$product.category.group_category.gender.name$": {
                [Op.substring]: q,
              },
            },
            {
            ...
            {
              "$product.category.group_category.slug$": {
                [Op.substring]: q,
              },
            },
            {
      ...
      const listId = listDetail.map((item) => item.product.id);
      let option = {
        where: {
          id: {
            [Op.in]: listId,
